...
const ProductList = () => {
  ...
  const fetchSubCategoryData = useCallback((selectedCategory) => {
    axios
      .get(`/api/fetch-subcategories?category_id=${selectedCategory}`, {
        headers: {
  ...
  const fetchSuperSubCategoryData = useCallback((selectedSubCategory) => {
    axios
      .get(
        `/api/fetch-supersubcategories?sub_category_id=${selectedSubCategory}`,
        {
          headers: {
            Authorization: localStorage.getItem("onlineKingToken"),
          },
  ...
  const fetchCarModelData = useCallback((brandId) => {
    axios
      .get(`/api/fetch-car-models?brand_id=${brandId}`)
      .then((res) => {
        if (res.data.code == 200) {
  ...
  const fetchCarYears = useCallback((modelId) => {
    axios
      .get(`/api/fetch-car-models?id=${modelId}`)
      .then((res) => {
        if (res.data.code === 200) {
  ...
  const [editorExchData, setExchEditorData] = useState("");
  const handleExchEditorChange = (data) => {
    setExchEditorData(data);
  };
  ...
  const handleAddProduct = () => {
    if (images.length === 0) {
      ...
      if (editorExchData) {
        formData.append("exchange_policy", editorExchData);
      }

      if (getProductData.cancellation_policy) {
        formData.append(
          "cancellation_policy",
          getProductData.cancellation_policy
        );
      }
      ...
      if(getProductData.pre_order_availability) {
        formData.append("pre_order_availability", getProductData.pre_order_availability)
        formData.append("pre_order_stock", getProductData.pre_order_stock);
        formData.append("pre_order_limit", getProductData.pre_order_limit)
        formData.append("estd_pre_order_processing_time", getProductData.estd_pre_order_processing_time)


      } else {
        formData.append("pre_order_availability", getProductData.pre_order_availability || false)
      }
      ...
      const features_associations = JSON.stringify(selectedFeatures);
      formData.append("features_associations", features_associations);
  ...
  const [searchQuerySuperSubCategory, setSearchQuerySuperSubCategory] =
    useState("");
  const [searchQueryBrand, setSearchQueryBrand] = useState("");

  const filteredRows = productData.filter(
    (e) =>
      ...
      (!searchQuerySubCategory ||
        (e.sub_category &&
          e.sub_category.sub_category_name
            ?.toLowerCase()
            .includes(searchQuerySubCategory.toLowerCase()))) &&
      (!searchQuerySuperSubCategory ||
        (e.super_sub_category &&
          e.super_sub_category.super_sub_category_name
            ?.toLowerCase()
            .includes(searchQuerySuperSubCategory.toLowerCase())))
  );
  ...
  const deleteProduct = (data) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `/api/delete-product?product_id=${data.id}`,
            {},
            {
              headers: {
                Authorization: localStorage.getItem("onlineKingToken"),
  ...
  const filteredProducts = getAllProductAttribute.filter(
  ...
  const handleProductChange = (event, value) => {
    ...
    setSelectedAttribute((prevData) => ({
      ...prevData,
      attributes: value.sort((a, b) => b.id - a.id).map((product) => ({
  ...
  const addedAttributeData = fieldData.map((combination) => {
    ...
    const preOrderLimit = getProductData.pre_order_availability && `${combination.combination}_pre_order_limit`;
    const preOrderStock = getProductData.pre_order_availability && `${combination.combination}_pre_order_stock`;
    ...
    return {
      combinations: combination.combination
        ...
        .map((attribute, index) => {
          ...
          const attributeId = selectedProductAttribute.find(
            (e) => e.attribute_name == attributeName.attribute_name
          );
          return {
      ...
      pre_order_stock: preOrderStock ? data[preOrderStock] : undefined
            ...
            <div className=" py-[10px] flex flex-col space-y-5">
              <div className="flex flex-col space-y-1">
                ...
                <span className="text-[#667085] font-[400] text-[16px]">
                  Simplify product management and presentation with Product
                  Setup, ensuring a streamlined and visually compelling
            ...
            <div className="flex items-center justify-between gap-[10px]">
              ...
              <div className="flex flex-col space-y-1 w-full">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSearchQuerySuperSubCategory(e.target.value)
                  }
                />
              </div>
              {/* <div className='flex flex-col space-y-1 w-full'>
            ...
            <div className="flex flex-col space-y-5  border border-[#EAECF0] rounded-[8px] p-[10px]">
              <div className="flex items-center px-3 justify-between">
                ...
                <div className="flex space-x-2">
                  <div
                    className="px-[16px] py-[10px] gap-[5px] flex items-center rounded-[8px] border border-[#D0D5DD] cursor-pointer hover:bg-[#cfaa4c] hover:text-[#fff] hover:border-none"
                  ...
                  <div
                    className="flex items-center gap-[5px] px-[18px] py-[10px] bg-[#cfaa4c] rounded-[8px] cursor-pointer hover:opacity-70"
                    onClick={handleAddNewProduct}
                  >
                    <MdAdd className="text-[#fff] text-[20px] font-[600]" />
                    <span className=" text-[14px] text-[#fff] font-[600]">
                      Add New Product
                    </span>
                  </div>
                </div>
                      ...
                      <TableBody>
                        {paginatedRows.map((row, i) => (
                          <TableRow key={i}>
                              ...
                              <img
                                src={
                                  row.images[0]?.image_url
                                    ? `${process.env.NEXT_PUBLIC_BASE_IMAGE_URL}${row.images[0].image_url}`
                                    : "/images/logo.png"
                                }
                                width={70}
                                height={50}
                                alt={row.product_name}
                                className="rounded-[8px] h-[50px] w-[50px]"
                              />
                            </TableCell>
                            ...
                            <TableCell>
                              <Switch
                                checked={row.is_topDeals}
                                //disabled={row.status}
                                onChange={() =>
                                  handleSwitchChange(row.id, "topDeals")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
                                    backgroundColor: row.is_topDeals
                                      ? "#CFAA4C"
                                      : "",
                                  },
                            ...
                            <TableCell>
                              <Switch
                                ...
                                //disabled={row.status}
                                onChange={() =>
                                  handleSwitchChange(row.id, "popular")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
                                    backgroundColor: row.is_popular
                                      ? "#CFAA4C"
                                      : "",
                                  },
                                  "& .Mui-checked + .MuiSwitch-track": {
                            ...
                            <TableCell>
                              <Switch
                                ...
                                //disabled={row.status}
                                onChange={() =>
                                  handleSwitchChange(row.id, "latest")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
                                    backgroundColor: row.is_latest
                                      ? "#CFAA4C"
                                      : "",
                                  },
                                  "& .Mui-checked + .MuiSwitch-track": {
                            ...
                            <TableCell>
                              <Switch
                                ...
                                onChange={() =>
                                  handleSwitchChange(row.id, "features")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
                                    backgroundColor: row.is_featured
                                      ? "#CFAA4C"
                                      : "",
                                  },
                                  "& .Mui-checked + .MuiSwitch-track": {
                            ...
                            <TableCell>
                              <Switch
                                checked={row.is_bestSelling}
                                onChange={() =>
                                  handleSwitchChange(row.id, "bestSelling")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
                                    backgroundColor: row.bestSelling
                                      ? "#CFAA4C"
                                      : "",
                                  },
                                  "& .Mui-checked + .MuiSwitch-track": {
                            ...
                            <TableCell>
                              <Switch
                                checked={row.status}
                                onChange={() =>
                                  handleSwitchChange(row.id, "active")
                                }
                                inputProps={{ "aria-label": "controlled" }}
                                sx={{
                                  "& .MuiSwitch-thumb": {
          ...
          <>
            <div className=" py-[10px] flex flex-col space-y-5">
              <div className="flex flex-col space-y-1">
                ...
                <span className="text-[#667085] font-[400] text-[16px]">
                  Introduce new items effortlessly with the Add New Product
                  feature in the admin application for a dynamic and up-to-date
                  online store.
                </span>
              </div>
            </div>
                ...
                <div className="flex flex-col border space-y-3 border-[#D0D5DD] rounded-[16px] p-[16px] w-[100%]">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-4">
                      ...
                      <input
                        type="checkbox"
                        checked={getProductData.pre_order_availability} // This should now reflect the updated state
                        className="outline-none focus:outline-none p-2"
                        name="pre_order_availability"
                        onChange={(e) => setGetProductData((prev) => ({...prev, [e.target.name]:e.target.checked}))} // Updates state on change
                      />
                    </div>

                    {getProductData.pre_order_availability && (
                      <>
                        <span className="text-[14px] text-[#344054] font-[500]">
            ...
            <div className="flex items-end justify-between gap-[30px]">
              <div className="flex flex-col space-y-3 border border-[#D0D5DD] rounded-[16px] p-[16px] w-[100%]">
                <span className="text-[18px] font-[600]">
                  Net Quantity and warranty info
                </span>
                <div className="flex flex-col space-y-1">
                  <span className="text-[14px] text-[#344054] font-[500]">
                    Net Quantity
              ...
              <div className="flex flex-col space-y-3 border border-[#D0D5DD] rounded-[16px] p-[16px] w-[100%]">
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <CustomEditor
                    ...
                    onChange={handleExchEditorChange}
                  />
                </div>
              </div>
              {/* <div className="flex flex-col space-y-3 border border-[#D0D5DD] rounded-[16px] p-[16px] w-[100%]">
            ...
            <div className="flex flex-col space-y-3 border border-[#D0D5DD] rounded-[16px] p-[16px] w-[100%]">
              ...
              <div className="flex flex-col space-y-3 w-full">
                ...
                {selectedAttribute.attributes &&
                  ...
                  selectedAttribute.attributes.map(
                          ...
                          <div className="flex-1">
                            ...
                            <FormControl fullWidth>
                              <span className="text-[10px] font-[500]">
                                Note: Enter{" "}
                                <span className="text-red-700">coma ( , )</span>{" "}
                                to create new {attribute.attribute_name}
                              </span>
                              <input
