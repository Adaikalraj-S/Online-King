...
const Banners = () => {
  ...
  const filteredProducts = getAllProducts?.filter((product) => product.status);
  ...
  const fetchCategory = async () => {
    try {
      ...
      console.log(getAllcategoryData, "gk");
  ...
  const handleFileChangeWebsite = (e) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        if (img.width === 1170 && img.height === 384) {
          setUploadedImageWebsite(e.target.result);
          setImageWeb(file);
        } else {
          openSnackbar("Please upload an image with dimensions 1170 × 384 pixels.", 'error');
          e.target.value = null;
  ...
  const deleteBanner = (data) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .post(
            `/api/delete-banner?banner_id=${data.id}`,
            {},
            {
              headers: {
                Authorization: localStorage.getItem("onlineKingToken"),
  ...
  const handleSwitchChange = (id) => {
    axios
      .post(
        `/api/toggle-banner-status?banner_id=${id}`,
        {},
        {
          headers: {
            Authorization: localStorage.getItem("onlineKingToken"),
        ...
        <div className=" py-[10px] flex flex-col space-y-5">
          <div className="flex flex-col space-y-1">
            ...
            <span className="text-[#667085] font-[400] text-[16px]">
              Elevate Visual Impact with Intuitive Banner Configuration in Admin
              Applications.
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-[10px]">
            <div className="flex flex-col space-y-1 w-full">
              ...
              <input
                ...
                value={bannerDetails.banner_name}
                className="inputText"
                id="banner_name"
                name="banner_name"
                onChange={getData}
              ...
              <>
                <div className="flex flex-col space-y-1 w-full">
                  ...
                  >
                    ...
                    {categoryData &&
                      categoryData
                        .filter((e) => e.status) // Filter active categories
                        .map((e, i) => (
                ...
                <div className="flex flex-col space-y-1 w-full">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedSuperSubCategory(e.target.value)
                    }
                  >
          ...
          <div className="flex items-baseline justify-between gap-[30px]">
            <div className="flex flex-col space-y-2  w-[100%]">
              ...
              <span className="text-red-600 font-[400] text-[10px]">
                Note: Only .png, .jpg, .jpeg format supported (Max. 2MB)
                (Dimesions : 1170px × 384px)
              </span>
                ...
                <div className="flex flex-wrap items-center mt-3">
                  {uploadedImageWebsite && (
                    <div className="p-2 relative">
                      <img
                        src={uploadedImageWebsite}
                        alt="Uploaded Website Banner"
          ...
          <div className="flex items-center gap-[24px] justify-end">
            <span className="resetButton" onClick={resetData}>
              Reset
            </span>
            <span className="submitButton" onClick={addBanner}>
              Submit
            </span>
          </div>
                    ...
                    <TableBody>
                      {paginatedRows.map((row, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell>
                            <img
                              src={`${process.env.NEXT_PUBLIC_BASE_IMAGE_URL}${row.web_image_url}`}
                              width={60}
                              height={60}
                              alt={row.banner_name}
                              className="rounded-[8px] h-[60px] w-[100px]"
                            />
                          </TableCell>
                          {/* <TableCell>
