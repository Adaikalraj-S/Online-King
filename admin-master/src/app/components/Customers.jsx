                ...
                <div className=' py-[10px] flex flex-col space-y-5'>
                    ...
                    <div className='flex items-end gap-[20px]'>
                        <div className='flex flex-col space-y-3 w-[50%]'>
                            ...
                            <div className='flex flex-col space-y-1' style={{ maxHeight: '210px', overflowY: 'auto' }}>
                            {orderDetailData?.user_addresses.map((orderDetail, index) => (
                                
                                <div  className='flex flex-col space-y-1 border rounded-lg ma-2 shadow p-[10px]'  key={index}>
                                    <span>GST No: {orderDetail?.gst_no}</span> 
                                    <span>Address Type: {orderDetail?.add_type}</span>
                                    <span>
                                    {orderDetail?.add1}, {orderDetail?.add2}
                                    </span>
                                    <span>{orderDetail?.landmark}</span>
                                    <span>{orderDetail?.area}</span>
                                    <span>{orderDetail?.city}</span>
                                    <span>{orderDetail?.state}</span>
                                    <span>{orderDetail?.pincode}</span>
