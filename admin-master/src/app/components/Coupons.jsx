  ...
  return (
    <div className='px-[20px]  container mx-auto overflow-y-scroll'>
      {!isEditable ?
              ...
              <>
                ...
                {specificDiscountType === 'Percent' && (
                  <div className='flex flex-col space-y-1 w-full'>
                    ...
                    <input type='text' placeholder='Enter discount in percentage' className='inputText' name='discount' value={specificDiscount} onChange={(e) => setSpecificDiscount(e.target.value)} />
                  </div>
                )}
                {specificDiscountType === 'Percent' && (
                  <div className='flex flex-col space-y-1 w-full'>
                        ...
                        <TableRow key={row.id} >
                          ...
                          {row.expiry_date && new Date(row.expiry_date) < new Date() ? (
                            <div className='flex items-center gap-[5px] py-[5px] bg-gray-200 rounded-[16px] justify-center'>
                              <Image src="/images/expire.png" height={10} width={10} alt='active' />
                          ...
                          <TableCell>
                            <Switch
                              checked={row.status === true}
                              onChange={() => handleSwitchChange(row.id)}
                              disabled={row.expiry_date && new Date(row.expiry_date) < new Date()} // Disable if expired
                              inputProps={{ 'aria-label': 'controlled' }}
                              sx={{
                                '& .MuiSwitch-thumb': {
                                  backgroundColor: row.status === true ? '#CFAA4C' : '',
        ...
        //           <div className='flex flex-col space-y-1 w-full'>
        //             <span>Percentage Discount</span>
        //             <input type='text' placeholder='Enter discount in percentage' defaultValue={editData.discount} className='inputText' name='discount' value={specificDiscount} onChange={(e) => setSpecificDiscount(e.target.value)} />
        //           </div>
        //         )}
        //         {specificDiscountType === 'Percent' && (
        //           <div className='flex flex-col space-y-1 w-full'>
        //             <span>Maximum Discount</span>
        //             <input type='text' placeholder='Enter Maximum discount' className='inputText' defaultValue={editData.max_discount} name='discount' value={specificMaxDiscount} onChange={(e) => setSpecificMaxDiscount(e.target.value)} />
        //           </div>
        //         )}
        //     <div className='flex flex-col space-y-1 w-full'>
        //       <span>Start Date </span>
        ...
        //   </div>
        // </div>
        <EditCouponForm editData={editData} customerData={fetchCouponData} goBack={resetButton}/>
      }
    </div>
  )
}
