...
const Page = ({ params }) => {
  ...
  const decodedParam =  decodeURIComponent(params.product_name.replaceAll("-SPLIT-", '/'));
  ...
  const finalPrice = (
    ...
    if (product?.is_offer_avl) {
      if (product?.offer_discount_type?.toLowerCase()?.trim() == "percent") {
        return convertInRupee(Number(defaultPrice) - Number(defaultPrice) * (Number(product?.offer_discount)/100));
      }
      else if (product?.offer_discount_type?.toLowerCase()?.trim() == "amount") {
        return convertInRupee(Number(defaultPrice) - Number(product?.offer_discount));
      }
    } else {
    ...
    <div>
      ...
      <div className="flex flex-col flex-grow font-fontNew">
            ...
            <div className="w-full">
              <div className="md:px-8 flex flex-col gap-4">
                ...
                <div className="flex gap-2 md:gap-4 items-center">
                  ...
                  <p className="text-red-600 text-xl md:text-2xl">
                    <span className="line-through">{convertInRupee(price)}</span> <span className="!no-underline text-gray-400 font-900 text-[14px]">(MRP)</span>
                  </p>
                </div>
                <div className="flex gap-2 md:gap-4 items-center">
                  <span className="!text-[14px] mt-[-15px] !font-[700]">Inclusive Of All Taxes</span>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 md:gap-7 md:items-center">
                  <div className="flex items-center gap-6 transition duration-300 cursor-pointer">
                    {(!combinationId && productDetails?.stock) ? (
                      <>
                    ...
                    ) : (combinationId && combinationStock) ? (
                      <>
                      <div className="flex gap-6 items-center">
                        <p className="font-bold text-base md:text-base py-2 md:py-4">
                          Quantity
                    ...
                    ) : (combinationId && productDetails?.product_attributes_associations?.find(e => e.id == combinationId)?.pre_order_limit && productDetails?.product_attributes_associations?.find(e => e.id == combinationId)?.pre_order_stock) ? (
                      <>
                      <div className="flex gap-6 items-center">
                        <p className="font-bold text-base md:text-base py-2 md:py-4">
                          Quantity
                    ...
                    ) : (!combinationId && productDetails?.pre_order_availability && productDetails?.pre_order_stock) ? (
                      <>
                      <div className="flex gap-6 items-center">
                        <p className="font-bold text-base md:text-base py-2 md:py-4">
                          Quantity
                      ...
                      <div className="text-[16px] text-[red] font-[500] drop-shadow-xl">OUT OF STOCK</div>
                    )}
        ...
        <div>
          ...
          <div className="px-4">
            <div>
              <RecommendedProductsCarousel productId={ productDetails.id}  />
            </div>
          </div>
        </div>
      </div>
