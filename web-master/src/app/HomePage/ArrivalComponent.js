...
import ProductAttributes from "../components/productDetails/attributes/ProductAttributes";
...
const ArrivalComponent = ({ productDetails }) => {
  ...
  //   }
  // }
  const finalPrice = (defaultPrice, discount = 0, discountType = "") => {
    if (discountType === "percent") {
      return defaultPrice - defaultPrice * (discount / 100);
    } else if (discountType === "amount") {
      return defaultPrice - discount;
    } else {
      // If discountType is not recognized, return the default price
      return defaultPrice;
    }
  };
  ...
  return (
    <div className="flex flex-col justify-evenly border-2 rounded-md w-full max-w-xs md:max-w-sm lg:max-w-md xl:max-w-lg  h-[35rem]   shadow-lg items-center">
      ...
      <div className="flex flex-col items-center justify-center p-4">
        <Link href={`/productDetails/${productDetails.id}`}>
          <p className="text-center font-semibold hover:underline">
        ...
        <div className="flex items-center mt-4 gap-[10px]">
          <div
            className="flex items-center bg-gray-300 justify-center gap-2 w-36 h-9 rounded-lg transition transform hover:scale-105 hover:shadow-lg duration-300 cursor-pointer"
            onClick={() =>
              addToCart(
                productDetails,
                selectedAttributes,
          ...
          >
            <Image src={addtocart} className="w-6 h-6" alt="Add to Cart" />
